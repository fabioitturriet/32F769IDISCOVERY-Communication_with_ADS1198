...
void CustomKeyboard::clearKeyboard() //add
{
		buffer[0] = 0;
...
void CustomKeyboard::setDataBuffer(Unicode::UnicodeChar* Data) //add
{
	Unicode::strncpy(buffer, Data, Unicode::strlen(Data)+1);
	buffer[Unicode::strlen(Data)+1] = 0;
	keyboard.setBufferPosition(Unicode::strlen(Data));
    firstCharacterEntry = false;
    uppercaseKeys = false;
    setKeyMappingList();
}
